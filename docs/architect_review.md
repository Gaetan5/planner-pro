...
- **Vite en mode dev sous Docker** : Monter le code source local comme volume (non implémenté actuellement dans le compose) permettrait le Hot Module Replacement (HMR) depuis le conteneur. En l'état, modifier le code du frontend nécessite de reconstruire l'image (`docker-compose up --build`).
  - _Recommandation_ : Ajouter des volumes de montage (`volumes: - ./frontend:/app - /app/node_modules`) dans le compose pour le développement local afin de bénéficier du HMR.
...
- _Recommandation_ : Configurer explicitement la taille des champs UUID avec `@db.VarChar(36)` pour économiser l'espace disque et accélérer les index.
...
- **Invalidation globale brute** : Supprimer l'intégralité de la clé `notes:${userId}` force un re-fetch complet de la liste des notes lors de la prochaine lecture. Si l'utilisateur a des centaines de notes, cela peut créer un pic de charge (phénomène de _Cache Stampede_).
  - _Recommandation_ : Pour un système à grande échelle, utiliser des clés par note individuelle (`note:${noteId}`) en plus de la liste, ou utiliser des structures Redis plus avancées (comme les Hashes) pour ne mettre à jour que la note modifiée.
...
2. **Fallback JWT trop permissif** :
   ...
   - _Recommandation_ : Retirer le fallback. Si `process.env.JWT_SECRET` est absent, le serveur doit lever une exception fatale et refuser de démarrer (Fail-Fast).
...
- Actuellement, l'instance Socket.io stocke les connexions et l'état des clients en mémoire locale. Si vous déployez demain plusieurs instances de votre backend derrière un répartiteur de charge (Load Balancer), un client connecté sur le serveur A ne recevra pas les événements émis sur le serveur B.
  - _Recommandation_ : Puisque Redis est déjà présent dans notre architecture, il suffit d'intégrer l'adaptateur Redis de Socket.io (`@socket.io/redis-adapter`). Cela permettra de diffuser les événements (comme `timer-started`) sur toutes les instances de backend de manière transparente.
