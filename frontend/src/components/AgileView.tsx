...
export const AgileView: React.FC = () => {
  ...
  const renderBurndownSvg = () => {
    ...
    const getX = (index: number) =>
      paddingLeft + (index / (pointsList.length - 1 || 1)) * graphWidth;
          ...
          {[0, 0.25, 0.5, 0.75, 1].map((ratio, i) => {
            ...
            return (
              <g key={i} className="grid-group">
                ...
                />
                <text x={paddingLeft - 10} y={y + 4} className="chart-axis-text chart-axis-text--y">
                  {val}
                </text>
              </g>
            );
            ...
            return (
              <g key={i} className="chart-point-group">
                ...
                {(i === 0 ||
                  ...
                  pointsList.length < 10 ||
                  i % Math.ceil(pointsList.length / 5) === 0) && (
                  <text
                    x={x}
      ...
      <div className="agile-header glass-panel">
        <div className="agile-header-title-container">
          ...
          <div>
            ...
            <p className="agile-subtitle">
              Planification intelligente, sprints fluides & graphiques de burndown
            </p>
          </div>
        </div>
        ...
        <div className="agile-header-controls">
          ...
          <button
            onClick={() => loadWorkspaceAgileData()}
            ...
            <div className="task-list">
              {backlogTasks.length === 0 ? (
                <div className="empty-column-state">
                  ...
                  <p className="subtext">
                    Toutes les tâches actives de vos projets sont planifiées dans des sprints ou
                    terminées.
                  </p>
                </div>
              ) : (
                      ...
                      <div className="story-point-container">
                        {editingTaskId === task.id ? (
                          <div className="sp-edit-form">
                            ...
                            />
                            <button
                              onClick={() => handleSaveStoryPoints(task.id)}
                              className="btn-save-sp"
                            >
                              <Check size={12} />
                            </button>
                          </div>
              ...
              {sprints.length === 0 ? (
                ...
                sprints.map((sprint) => {
                  ...
                  return (
                    <div
                      key={sprint.id}
                      className={`sprint-card glass-panel sprint-card--${sprint.status.toLowerCase()}`}
                    >
                      <div className="sprint-card-header">
                        <div>
                          <h3 className="sprint-card-name">{sprint.name}</h3>
                          <span
                            className={`sprint-status-badge badge--${sprint.status.toLowerCase()}`}
                          >
                        ...
                        <div className="sprint-points-summary">
                          <span className="points-total">
                            {completedSprintPoints} / {sprintPoints} SP
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="sprint-task-list">
                        {sprintTasks.length === 0 ? (
                          <p className="empty-sprint-tasks">
                            Aucune tâche planifiée dans ce sprint.
                          </p>
                        ) : (
                          sprintTasks.map((task) => (
                            <div key={task.id} className="sprint-task-item">
                              <div className="task-item-left">
                                <span
                                  className={`status-dot status-dot--${task.status.toLowerCase()}`}
                                ></span>
                ...
                <div className="kpi-card">
                  ...
                  <div className="kpi-info">
                    ...
                    <span className="kpi-value">
                      {currentSprintTasks.filter((t) => t.status === 'DONE').length} /{' '}
              ...
              <div className="picker-list">
                {sprints.map((s) => (
                  ...
                  >
                    ...
                    <span
                      className={`picker-btn-status picker-btn-status--${s.status.toLowerCase()}`}
                    >
                      {s.status === 'ACTIVE'
                        ? 'Actif'
                        : s.status === 'COMPLETED'
