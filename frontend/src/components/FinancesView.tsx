...
import { DollarSign, TrendingUp, TrendingDown, Clock, AlertTriangle, PiggyBank, BarChart3 } from 'lucide-react'
      ...
      <div className="finances-layout">
        ...
        <div className="glass-panel finances-empty">
          ...
          <span className="finances-empty__hint">
            Les données financières apparaîtront lorsque vos projets auront un budget défini,
            des profils de ressources avec des taux horaires, et du temps logué sur les tâches.
          </span>
        </div>
      </div>
    )
    ...
    <div className="finances-layout">
      ...
      <div className="finances-summary-grid">
        <div className="glass-panel finances-summary-card finances-summary-card--revenue">
          ...
          <span className="finances-summary-card__value">{formatCurrency(summary.totalRevenue)}</span>
          <span className="finances-summary-card__badge finances-summary-card__badge--neutral">
            <BarChart3 size={10} /> {summary.projects.length} projet{summary.projects.length > 1 ? 's' : ''}
          </span>
        </div>
        ...
        <div className="glass-panel finances-summary-card finances-summary-card--margin">
          ...
          <span className={`finances-summary-card__value ${summary.totalMargin >= 0 ? 'finances-summary-card__value--positive' : 'finances-summary-card__value--negative'}`}>
            {formatCurrency(summary.totalMargin)}
          </span>
          <span className={`finances-summary-card__badge ${summary.totalMarginPercent >= 0 ? 'finances-summary-card__badge--positive' : 'finances-summary-card__badge--negative'}`}>
            {summary.totalMarginPercent >= 0 ? <TrendingUp size={10} /> : <TrendingDown size={10} />}
        ...
        <div className="glass-panel finances-summary-card finances-summary-card--budget">
          ...
          <span className="finances-summary-card__value">{formatCurrency(summary.totalBudget)}</span>
        </div>
      </div>
            ...
            <tbody>
              {summary.projects.map((proj) => (
                <tr key={proj.projectId}>
                  ...
                  <td>
                    <span style={{ color: proj.marginCents >= 0 ? 'var(--color-success)' : 'var(--color-error)', fontWeight: 600 }}>
                      {proj.marginPercent}%
                    </span>
                  </td>
                  <td>
                  ...
                  <td>
                    {proj.budgetCents ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <span style={{ fontSize: 'var(--font-2xs)', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>
                          {proj.burnPercent}%
                        </span>
                      </div>
                    ) : '—'}
