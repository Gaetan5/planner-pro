      ...
      <div className="copilot-widget-header">
        ...
        <div className="copilot-tab-bar">
          ...
          >
            Alertes de Risques
            {alerts.length > 0 && (
              <span className={`alerts-count-badge ${alerts.some(a => a.severity === 'CRITICAL') ? 'critical' : ''}`}>
                {alerts.length}
              </span>
            )}
          </button>
          ...
          <div className="copilot-briefing-tab-content">
            {!briefing && !loadingBriefing ? (
              <div className="copilot-briefing-placeholder">
                ...
                <p>Prêt à démarrer votre journée ? Laissez l'IA analyser vos tâches et priorités pour rédiger votre briefing.</p>
                <button className="btn-primary copilot-generate-btn" onClick={handleGenerateBriefing}>
              ...
              <div className="copilot-briefing-display-wrapper">
                ...
                <div className="briefing-actions-panel">
                  <div className="tts-controls">
                    {!isPlayingTts ? (
                      <button className="tts-action-btn" onClick={startTts} title="Écouter le briefing">
                        <Volume2 size={16} />
                        <span>Écouter</span>
                      </button>
                    ) : (
                      <button className="tts-action-btn active" onClick={pauseTts} title="Suspendre l'écoute">
                        <Pause size={16} />
                        <span>Pause</span>
                      </button>
                    )}
                    {(isPlayingTts || isPausedTts) && (
                      <button className="tts-action-btn stop" onClick={stopTts} title="Arrêter l'écoute">
                        <Square size={14} />
                      </button>
                    )}
                  </div>

                  <button className="briefing-refresh-btn" onClick={handleGenerateBriefing} title="Regénérer le briefing">
                    <RefreshCw size={14} />
                    <span>Regénérer</span>
                  </button>
                </div>
                ...
                <div className="briefing-text-content">
                  {(briefing || '').split('\n').map((line, idx) => {
                    ...
                    if (cleanLine.startsWith('📅') || cleanLine.startsWith('⚠️') || cleanLine.startsWith('💡')) {
                      return <h4 key={idx} className="briefing-section-title">{cleanLine}</h4>
                    }
                    if (cleanLine.startsWith('-') || cleanLine.startsWith('*')) {
                      return <li key={idx} className="briefing-list-item">{cleanLine.replace(/^[-*]\s*/, '')}</li>
                    }
                    return <p key={idx} className="briefing-para-text">{cleanLine}</p>
                  })}
                </div>
              </div>
            )}
          ...
          <div className="copilot-alerts-tab-content">
            {loadingAlerts ? (
              ...
              <div className="copilot-alerts-empty-state">
                ...
                <p>Aucun retard, aucune surcharge ni dépendance bloquée n'ont été détectés sur ce workspace.</p>
              </div>
            ) : (
              <div className="copilot-alerts-list">
                <div className="alerts-meta">
                  <span>Moteur de règles heuristiques actif</span>
                  <button className="alerts-refresh-icon-btn" onClick={fetchAlerts} title="Rafraîchir les alertes">
                    <RefreshCw size={12} />
                  </button>
                </div>

                {alerts.map((alert) => (
                  <div key={alert.id} className={`copilot-alert-card ${alert.severity.toLowerCase()}`}>
