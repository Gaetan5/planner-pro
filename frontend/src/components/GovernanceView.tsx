...
export const GovernanceView: React.FC = () => {
  ...
  const [delStatus, setDelStatus] = useState<
    'DRAFT' | 'READY_FOR_REVIEW' | 'ACCEPTED' | 'DELIVERED'
  >('DRAFT');
  ...
  const acceptedDeliverablesCount = allDeliverables.filter(
    (d) => d.status === 'ACCEPTED' || d.status === 'DELIVERED',
  ).length;
  ...
  const isReadyForDelivery =
    ...
    acceptedDeliverablesCount === totalDeliverablesCount;
        ...
        <div className="workspace-info-card">
          ...
          <div>
            ...
            <p className="workspace-meta">
              Propriétaire :{' '}
              {workspaceMembers.find((m) => m.role === 'OWNER')?.user.name || 'Admin'}
            </p>
          </div>
        </div>
      ...
      {projects.length === 0 ? (
        <div className="empty-state-gov">
          ...
          <p className="empty-state-desc">
            Créez un projet dans le module Kanban pour commencer à gérer vos jalons et livrables.
          </p>
        </div>
      ) : (
              ...
              <div className="project-selector-list">
                {projects.map((proj) => (
                  ...
                  >
                    ...
                    <span
                      className={`project-item-status badge badge--${(proj.status || 'PLANNING').toLowerCase().replace('_', '')}`}
                    >
          ...
          <main className="main-governance-panel">
            ...
            {activeSubTab === 'project' && (
              <>
                ...
                {activeProject && (
                  <div
                    className="gov-card project-overview-premium-card"
                      ...
                      <div className="project-overview-info">
                        ...
                        <div className="project-quick-stats">
                          ...
                          <div className="quick-stat-item">
                            ...
                            <span className="quick-stat-value">
                              <NumberTicker value={acceptedDeliverablesCount} /> /{' '}
                        ...
                        activeProject.deliverables.map((del) => (
                          <div key={del.id} className="gov-item">
                            <div className="gov-item-left">
                              ...
                              {del.description && (
                                <p className="gov-item-desc">{del.description}</p>
                              )}
                              {del.dueDate && (
                                <span className="gov-item-meta">
                      ...
                      <div className="form-grid">
                        ...
                        <div className="form-group">
                          <label className="form-label">
                            Checklist de validation (un item par ligne)
                          </label>
                          <textarea
                            required
                  ...
                  <div className="gov-list" style={{ maxHeight: 'none' }}>
                    {!activeProject?.deliveries || activeProject.deliveries.length === 0 ? (
                      <div className="empty-state-gov" style={{ padding: '20px' }}>
                        <p className="workspace-meta text-center">
                          Aucune livraison en cours ou passée.
                        </p>
                        {totalTasksCount > 0 && !isReadyForDelivery && (
                          <p
                            className="workspace-meta text-center"
                            style={{ fontSize: '11px', color: 'var(--color-warning)' }}
                          >
                            ⚠️ Pour débloquer la livraison, terminez toutes les tâches de ce projet
                            et validez l'ensemble de ses livrables.
                          </p>
                        )}
                      </div>
                    ) : (
                      activeProject.deliveries.map((dev) => (
                        <div
                          key={dev.id}
                          className="delivery-record-card"
                          style={{ marginBottom: '10px' }}
                        >
                          <div className="delivery-record-header">
                            <div>
                              <span
                                className={`badge badge--${dev.status.toLowerCase().replace(/_/g, '')}`}
                              >
                                Livraison : {dev.status}
                              </span>
                              <span className="workspace-meta" style={{ marginLeft: '12px' }}>
                                Créée le :{' '}
                                {new Date(dev.deliveredAt || new Date()).toLocaleDateString()}
                              </span>
                            </div>
                            <div className="form-actions" style={{ margin: 0, gap: '8px' }}>
                              {dev.status === 'DRAFT' && (
                                <button
                                  onClick={() =>
                                    updateDeliveryStatus(dev.id, 'READY_FOR_ACCEPTANCE')
                                  }
                          ...
                          {dev.summary && (
                            <p
                              className="gov-item-desc"
                              style={{ marginBottom: '15px', whiteSpace: 'pre-wrap' }}
                            >
                              {dev.summary}
                            </p>
                          )}
                            ...
                            <div>
                              ...
                              <div className="checklist-container">
                                {dev.checklist.map((item) => (
                                  ...
                                  >
                                    <div
                                      className={`checklist-item-checkbox ${item.checked ? 'checklist-item-checkbox--checked' : ''}`}
                                    >
                                      {item.checked && <Check size={10} strokeWidth={3} />}
                                    </div>
                                    <span
                                      className={`checklist-item-text ${item.checked ? 'checklist-item-text--checked' : ''}`}
                                    >
                                      {item.title}
                                    </span>
                                  </div>
