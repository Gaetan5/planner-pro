...
export const GanttView: React.FC = () => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (interaction.type === 'move') {
        ...
        const maxDelta = Math.floor(
          (interaction.initialDueDate.getTime() - interaction.initialStartDate.getTime() - (1000 * 60 * 60 * 24)) /
          (1000 * 60 * 60 * 24)
        )
        ...
        const minDelta = Math.ceil(
          (interaction.initialStartDate.getTime() - interaction.initialDueDate.getTime() + (1000 * 60 * 60 * 24)) /
          (1000 * 60 * 60 * 24)
        )
  ...
  useEffect(() => {
    ...
    allTasks.forEach((task, taskIndex) => {
      ...
      task.dependencies.forEach(dep => {
        ...
        const path = `M ${xParentEnd} ${yParent} ` +
                     `L ${xMid} ${yParent} ` +
                     `L ${xMid} ${yTask} ` +
                     `L ${xTaskStart} ${yTask}`
  ...
  return (
    <div className="gantt-view-container">
      ...
      <div className="gantt-header glass-panel">
        <div className="gantt-header-title-container">
          ...
          <div>
            ...
            <p className="gantt-subtitle">Ajustez vos plannings visuellement & propagez automatiquement le travail</p>
          </div>
        </div>
      ...
      {allTasks.some(t => t.dependencies?.some(d => {
        ...
        <div className="gantt-conflict-banner glass-panel">
          ...
          <span>Certaines tâches commencent avant la fin de leur prédécesseur. Les dépendances en conflit clignotent en rouge.</span>
        </div>
      )}
              ...
              allTasks.map(task => (
                <div key={task.id} className="task-meta-row">
                  <div className="task-meta-left">
                    <span className={`task-status-dot task-status-dot--${task.status.toLowerCase()}`}></span>
                    <span className="task-meta-title" title={task.title}>{task.title}</span>
                  </div>
                  <div className="task-meta-actions">
                    <button
                      ...
                      title={linkingTaskId ? "Sélectionner la tâche parente" : "Lier cette tâche à une autre"}
                    >
                      <Link size={12} />
                    </button>
        ...
        <div className="gantt-right-panel" ref={timelineScrollRef}>
          ...
          <div className="gantt-timeline-header" style={{ width: datesArray.length * cellWidth }}>
            {datesArray.map((date, idx) => {
              ...
              return (
                <div key={idx} className={`header-date-cell ${isToday ? 'header-date-cell--today' : ''}`} style={{ width: cellWidth }}>
                  {zoomMode === 'day' ? (
                    <>
                    ...
                    <span className="week-label">S. {Math.ceil(dayNum / 7) + date.getMonth() * 4}</span>
                  )}
                </div>
              )
            })}
          ...
          <div className="gantt-timeline-body" style={{ width: datesArray.length * cellWidth }}>
            ...
            <svg className="gantt-svg-overlay" style={{ width: datesArray.length * cellWidth, height: gridHeaderHeight + allTasks.length * 56 }}>
              <defs>
