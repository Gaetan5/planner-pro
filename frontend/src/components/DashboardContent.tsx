      ...
      {activeTimer ? (
        <div className="glass-panel pulse-timer timer-bar timer-bar--active">
          <div className="timer-bar-left">
            ...
            <div className="timer-info-text">
              ...
              <h2 className="timer-info-title">
                {activeTimer.task?.title || 'Chronométrage actif'}
              </h2>
            </div>
          </div>
        ...
        <div className="glass-panel timer-bar timer-idle-bar">
          <div className="timer-idle-left">
            ...
            <p className="timer-idle-text">
              Aucun chronomètre en cours. Démarrez-en un depuis le module Kanban.
            </p>
          </div>
      ...
      <div className="dashboard-grid">
        ...
        <section className="glass-panel dashboard-widget">
          ...
          <div className="stat-grid">
            ...
            <div className="glass-panel stat-card">
              <span className="stat-card-number stat-card-number--primary">
                {inProgressTasks.length}
              </span>
              <p className="stat-card-label">En Cours</p>
            </div>
            <div className="glass-panel stat-card">
              <span className="stat-card-number stat-card-number--success">
            ...
            <div className="urgent-tasks-list">
              {allTasks
                ...
                .map((task) => (
                  <div
                    key={task.id}
                    className={`urgent-task-item ${task.priority === 'HIGH' ? 'urgent-task-item--high' : ''}`}
                  >
            ...
            {timeBlocks.length === 0 ? (
              <div className="schedule-empty">
                ...
                <button
                  ...
                  className="btn-primary schedule-empty-btn"
                >
                  Time-blocker une tâche
                </button>
              </div>
            ) : (
              timeBlocks.map((tb) => {
                const start = new Date(tb.startTime).toLocaleTimeString([], {
                  hour: '2-digit',
                  minute: '2-digit',
                });
                return (
                  <div key={tb.id} className="schedule-card">
                    ...
                    <div className="schedule-details">
                      ...
                      <p className="schedule-project-name">
                        Projet : {tb.task?.project?.name || 'Inbox'}
                      </p>
                    </div>
                  </div>
                );
